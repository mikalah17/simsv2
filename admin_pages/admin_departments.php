    ...
    if (isset($_POST['action'])) {
        try {
            if ($_POST['action'] === 'add') {
                ...
                // Get next department_id
                $result = $pdo->query('SELECT MAX(department_id) AS maxId FROM dept');
            ...
            elseif ($_POST['action'] === 'update') {
                ...
                if (!$department_id || !$name) {
                    throw new Exception('Invalid department ID or name');
                }
            ...
            elseif ($_POST['action'] === 'delete') {
                ...
                $check = $pdo->prepare('SELECT COUNT(*) as count FROM employee WHERE department_id = ?');
...
// Get all departments
$stmt = $pdo->query('SELECT department_id, department_name FROM dept ORDER BY department_name');
...
<body>
        ...
        <div class="content-wrapper">
                ...
                <div class="scroll-area" id="itemsList">
                    <?php if (!empty($departments)): ?>
                        <?php foreach ($departments as $dept): ?>
                            <div class="inner-card" data-name="<?php echo htmlspecialchars($dept['department_name']); ?>">
                                <div class="item-details">
                                    <span><b>Department:</b> <?php echo htmlspecialchars($dept['department_name']); ?></span>
                                </div>
                                <button class="edit-btn" onclick="showEditPanel(<?php echo $dept['department_id']; ?>, '<?php echo htmlspecialchars($dept['department_name']); ?>')">Edit</button>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <p style="color: rgba(255,255,255,0.7); text-align: center; padding: 20px;">No departments found</p>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="right-panel" id="addPanel">
                ...
                <form method="POST">
                    ...
                    <button type="submit" name="action" value="add" class="add-btn">Add Department</button>
                </form>
            </div>
    ...
    <script>
        ...
        function showEditPanel(deptId, deptName) {
            document.getElementById('addPanel').classList.add('hidden');
            document.getElementById('editPanel').classList.remove('hidden');

            document.getElementById('editItemId').value = deptId;
            document.getElementById('editItemName').value = deptName;
        }

        function saveItem() {
            const deptId = document.getElementById('editItemId').value;
            ...
            form.innerHTML = `
                ...
                <input type="hidden" name="department_name" value="${name.replace(/"/g, '&quot;')}">
            `;
        ...
        function deleteItem() {
            const deptName = document.getElementById('editItemName').value;
            const deptId = document.getElementById('editItemId').value;
